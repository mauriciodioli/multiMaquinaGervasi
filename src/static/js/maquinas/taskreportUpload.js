...
// Variante per TaskReport RTF (HypCut)
(function () {
  const btnOpen = document.getElementById('btn-subir-taskreport-rtf');
  const btnDo   = document.getElementById('btn-confirmar-taskreport-rtf-upload');
  const result  = document.getElementById('taskreport-upload-result');

  const inputFile    = document.getElementById('taskreport-rtf-file');
  const inputMachine = document.getElementById('taskreport-rtf-machine');
  const inputTask    = document.getElementById('taskreport-rtf-taskname');
  const selectMode   = document.getElementById('taskreport-rtf-mode');

  const modalEl = document.getElementById('modalLaserTaskReportUploadRtf');
  ...
  btnDo.addEventListener('click', async () => {
    try {
      if (!uploadUrl) {
        setResult('❌ uploadUrl mancante (data-upload-url RTF).', false);
        return;
      }

      const file = inputFile?.files?.[0];
      if (!file) {
        setResult('❌ Seleziona un file RTF.', false);
        return;
      }
      ...
      const fd = new FormData();
      fd.append('file', file);
      fd.append('machine', (inputMachine?.value || '').trim());
      fd.append('task_name', (inputTask?.value || 'HypCut').trim());
      fd.append('mode', (selectMode?.value || 'upsert').trim());
      ...
      if (!resp.ok || data.status !== 0) {
        setResult(`❌ Import RTF fallito: ${data.msg || resp.status}`, false);
        return;
      }

      setResult(`✅ OK RTF — inserted: <b>${data.inserted}</b>, updated: <b>${data.updated}</b>, skipped: <b>${data.skipped}</b>`, true);
      closeModal();

    } catch (e) {
      setResult(`❌ Errore RTF: ${e}`, false);
    } finally {
      btnDo.disabled = false;
      btnDo.innerText = 'Importa';
    }
